...
% Chun-Yi Wu
    ...
    properties
        ...
        tp;     % [double] time of periapsis passage
    ...
    methods
        ...
        function op = OrbitParam(varargin)
            switch ( nargin )
                ...
                case 12
                    ...
                    op.name1 = varargin{11};
                    op.name2 = varargin{12};
        ...
        figure(fnum); clf(fnum); hold on; axis equal; grid on;
        ...
        figure(fnum); clf(fnum); hold on; grid on;
        ...
        function [] = plotDistance(op,fnum,t0,tf)
        %% 
        % plotDistance
        %
        ...
        if ( max(rs)-min(rs) < 500 )
            plot(ts,rs,'b-');
            plot(ts,ones(size(ts))*(op.R1+op.R2),'r:');
            datetick('x');
            xlabel('time');
            ylabel('distance [m]');
            ...
            plot(ts,rs/1000,'b-');
            plot(ts,ones(size(ts))*(op.R1+op.R2)/1000,'r:');
            datetick('x');
            xlabel('time');
            ylabel('distance [km]');
            title(sprintf('Distance - %s',op.name1));
